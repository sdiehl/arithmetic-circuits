...
testSplitUnsplitCircuit nbits
  = ArithCircuit [ Split (InputWire 0) midWires
...
arbAffineCircuitWithMids
  :: Arbitrary f
  => Int -> [Int] -> Int -> Gen (AffineCircuit Wire f)
...
arbArithCircuit
  :: Arbitrary f
  => (Int, Int, Int) -- ^ distribution of frequency of mul/equal/split
                     -- gates, respectively
  -> Int
  -> Int
  -> Gen (ArithCircuit f)
arbArithCircuit (distMul, distEqual, distSplit) numInps size
  ...
  frequency . catMaybes $ [ (distMul,) <$> mulGate gates mids
